...
/** Try the best way */
define('WATERMARKEDLIB_DISPLAY_AUTO', 0);
/** Display using object tag */
define('WATERMARKEDLIB_DISPLAY_EMBED', 1);
/** Display inside frame */
define('WATERMARKEDLIB_DISPLAY_FRAME', 2);
/** Display normal link in new window */
define('WATERMARKEDLIB_DISPLAY_NEW', 3);
/** Force download of file instead of display */
define('WATERMARKEDLIB_DISPLAY_DOWNLOAD', 4);
/** Open directly */
define('WATERMARKEDLIB_DISPLAY_OPEN', 5);
/** Open in "emulated" pop-up without navigation */
define('WATERMARKEDLIB_DISPLAY_POPUP', 6);

/** Legacy files not needed or new resource */
define('WATERMARKEDLIB_LEGACYFILES_NO', 0);
/** Legacy files conversion marked as completed */
define('WATERMARKEDLIB_LEGACYFILES_DONE', 1);
/** Legacy files conversion in progress*/
define('WATERMARKEDLIB_LEGACYFILES_ACTIVE', 2);
 ...
 */
function watermarkedlib_try_file_migration($filepath, $cmid, $courseid, $component, $filearea, $itemid) {
    $fs = get_file_storage();
 ...
 */
function watermarkedlib_get_displayoptions(array $enabled, $current=null) {
    if (is_number($current)) {
        $enabled[] = $current;
    }

    $options = array(WATERMARKEDLIB_DISPLAY_AUTO     => get_string('resourcedisplayauto'),
                     WATERMARKEDLIB_DISPLAY_EMBED    => get_string('resourcedisplayembed'),
                     WATERMARKEDLIB_DISPLAY_FRAME    => get_string('resourcedisplayframe'),
                     WATERMARKEDLIB_DISPLAY_NEW      => get_string('resourcedisplaynew'),
                     WATERMARKEDLIB_DISPLAY_DOWNLOAD => get_string('resourcedisplaydownload'),
                     WATERMARKEDLIB_DISPLAY_OPEN     => get_string('resourcedisplayopen'),
                     WATERMARKEDLIB_DISPLAY_POPUP    => get_string('resourcedisplaypopup'));
    ...
    if (empty($result)) {
        ...
        $result[WATERMARKEDLIB_DISPLAY_OPEN] = $options[WATERMARKEDLIB_DISPLAY_OPEN];
    }
 ...
 */
function watermarkedlib_guess_url_mimetype($fullurl) {
    global $CFG;
 ...
 */
function watermarkedlib_get_extension($fullurl) {
 ...
 * @return string html
 */
function watermarkedlib_embed_image($fullurl, $title) {
    $code = '';
    $code .= '<div class="watermarkedcontent watermarkedimg">';
    $code .= "<img title=\"".s(strip_tags(format_string($title)))."\" class=\"watermarkedimage\" src=\"$fullurl\" alt=\"\" />";
    $code .= '</div>';
 ...
 * @return string html
 */
function watermarkedlib_embed_pdf($fullurl, $title, $clicktoopen) {
    global $CFG, $PAGE;

    $code = <<<EOT
<div class="watermarkedcontent watermarkedpdf">
  <object id="watermarkedobject" data="$fullurl" type="application/pdf" width="800" height="600">
...
EOT;
    ...
    $PAGE->requires->js_init_call('M.util.init_maximised_embed', array('watermarkedobject'), true);
 ...
 * @return string html
 */
function watermarkedlib_embed_general($fullurl, $title, $clicktoopen, $mimetype) {
    global $CFG, $PAGE;
    ...
    // this is ok in HTML5.
    $code = <<<EOT
<div class="watermarkedcontent watermarkedgeneral">
  <iframe id="watermarkedobject" src="$fullurl">
    $clicktoopen
  </iframe>
</div>
EOT;
